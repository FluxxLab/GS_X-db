...
async castVote(delegateId: string, entryId: string): Promise<{voteCount: number; counted: boolean}> {
    ...
    if(counted){
        ...
        this.realtime.emitToRoom(Rooms.voting, 'voting:tally', {entryId, voteCount});
    }else {
...
async leaderboard(limit: number = 10): Promise<Array<{ entry: PitchEntry; voteCount: number }>> {

    const all = await this.listEntries();
    return all
    .sort((a,b) => b.voteCount - a.voteCount)
    .slice(0, limit)
    .map(({voteCount, ...entry}) => ({entry: entry as PitchEntry, voteCount}));
}
