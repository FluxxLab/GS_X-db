...
import { TriviaOption, TriviaQuestion, TriviaStatus } from './entities/trivia-question.entity';
...
export class TriviaService{
    ...
    async pushLive(id: string): Promise<TriviaQuestion> {
         ...
         * 
         */
        await this.questions.update({status: TriviaStatus.LIVE}, {status: TriviaStatus.CLOSED});
   
        question.status = TriviaStatus.LIVE;
        const saved = await this.questions.save(question);
        this.realtime.emitToRoom(Rooms.trivia, 'trivia:question', this.toDelegateShape(saved));
        return saved;
    }
     ...
     * Internals
     */
    private async distribution(questionId: string): Promise<Record<TriviaOption, number>> {
