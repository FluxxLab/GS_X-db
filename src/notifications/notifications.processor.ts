...
export class NotificationsProcessor extends WorkerHost {
    ...
    async process(job: Job<{notificationId: string}>):Promise<void>{
        ...
        (await this.deviceTokens.findBy({delegateId: In(delegateIds)})).map((t) => t.token) : [];
        
        const {invalidTokens} = await this.push.sendToTokens(tokens, notification.title, notification.body);
