...
export class DelegatesService {
    ...
    searchDelegates(q: string, limit: number): Promise<Delegate[]>{
        return this.delegateRepository
           ...
           .where('d.name ILIKE :q OR d.organisation ILIKE :q OR d.country ILIKE :q',{q: `%${q}%`})
           .take(limit)
           .getMany();
    }
    ...
    async idsForSegment(segment: string): Promise<string[]>{
        ...
        switch (segment) {
            ...
            case AudienceSegment.VIP:
                qb.where('d.accessTier IN (:...tiers)',{tiers: [AccessTier.VIP, AccessTier.VVIP]});
                break;
     ...
     */
    async matchRegistration(email: string, inviteCode?: string): Promise<RegistrationEntry | null>{
    ...
    async createDelegate(input: {name: string; email: string; passwordHash: string; accessTier: AccessTier; pendingReview: boolean; phone: string | null; consentAt: Date;}):Promise<Delegate>{
    ...
    async addRegistrationEntry(dto: CreateRegistrationEntryDto):Promise<RegistrationEntry>{
        return this.registrationRepository.save(this.registrationRepository.create({
            email: dto.email?.toLowerCase() ?? null,
            inviteCode: dto.inviteCode ?? randomBytes(4).toString('hex').toUpperCase(),
            name: dto.name ?? null,
    ...
    async exportCsv(): Promise<string>{
        const rows = await this.delegateRepository.find({ order: { createdAt: 'ASC' } });
        const esc = (v: unknown) => `"${String(v ?? '').replace(/"/g, '""')}"`;
        const header = 'name,email,organisation,tier,tracks,interests,consentAt,registeredAt';
        const lines = rows.map((d) =>
            [d.name, d.email, d.organisation, d.accessTier,
            d.tracks.join('; '), d.interests.join('; '),
            d.consentAt?.toISOString() ?? '', d.createdAt.toISOString()].map(esc).join(','),
        );
        return [header, ...lines].join('\n');
    }

    async namesByIds(ids: string[]): Promise<Map<string,{name: string; organisation: string | null}>>{
        if(ids.length === 0){
            return new Map();
        }
