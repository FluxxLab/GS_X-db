...
export class CreateRegistrationEntryDto {
    @ApiPropertyOptional({description: 'Match key: applicant email (invite code auto-generated if omitted)'})
    @IsOptional()
    @IsEmail()
    email?: string;

    @ApiPropertyOptional({ maxLength: 255, description: 'Match key: printed invite code'})
    @IsOptional()
    @IsString()
    @MaxLength(255)
    inviteCode?: string;
