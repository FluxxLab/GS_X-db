...
export class SessionsService {
    ...
    findLiveInRoom(room: string): Promise<Session | null> {
        return this.sessions.findOne({ where: { room, status: SessionStatus.LIVE } });
    }
    ...
    async create(dto: CreateSessionDto):Promise<Session>{
        ...
        const session = this.sessions.create({
            ...
            speakers: speakerIds?.length ? await this.speakers.findBy({
                id: In(speakerIds),
            }) : [],
        });
    ...
    searchSessions(q: string, limit: number = 10): Promise<Session[]>{
       return this.sessions
            ...
            .where('s.title ILIKE :q OR s.description ILIKE :q OR sp.role ILIKE :q', { q: `%${q}%` })
            .take(limit)
            .getMany();
    }
